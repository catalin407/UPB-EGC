...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(5, 0.5f, 5));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(2, 2, 2));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(45), glm::vec3(1, 1, 1));
        RenderMesh(meshes["box"], shaders["Simple"], modelMatrix);
    }
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(5, 2, -7));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(2.2f));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(-45), glm::vec3(1, 1, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_UP) && projectionMode)
    {
        top += deltaTime;
        bottom -= deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, nearZ, farZ);
    }
    if (window->KeyHold(GLFW_KEY_DOWN) && projectionMode)
    {
        top -= deltaTime;
        bottom += deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, nearZ, farZ);
    }
    if (window->KeyHold(GLFW_KEY_LEFT) && projectionMode)
    {
        left -= deltaTime;
        right += deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, nearZ, farZ);
    }
    if (window->KeyHold(GLFW_KEY_RIGHT) && projectionMode)
    {
        left += deltaTime;
        right -= deltaTime;
        projectionMatrix = glm::ortho(left, right, bottom, top, nearZ, farZ);
